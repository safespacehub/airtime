 ...
 * Based on Nordic Semiconductor GPS sample (assistance_supl.c)
 *
...
LOG_MODULE_REGISTER(supl_assist, CONFIG_LOG_DEFAULT_LEVEL);
...
static int supl_logger(int level, const char *fmt, ...)
{
	...
	if (ret < 0) {
		...
		/* Message truncated, but continue */
	}

	/* Only log important messages, reduce verbosity */
	/* Skip verbose messages like "Bytes received", "Bytes sent", "ULP encoding length" */
	if (strstr(buffer, "Bytes received") != NULL ||
	    strstr(buffer, "Bytes sent") != NULL ||
	    strstr(buffer, "ULP encoding length") != NULL ||
	    strstr(buffer, "ULP ossDecode more input") != NULL) {
		/* These are too verbose, skip them */
		return ret;
	}
...
int supl_assist_init(struct k_work_q *assistance_work_q)
{
	...
	LOG_INF("SUPL assistance initialized (server: %s:%d)", SUPL_SERVER, SUPL_SERVER_PORT);
	return 0;
}
